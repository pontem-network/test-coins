...
module test_coins::coins_extended_tests {
    ...
    use test_coins_extended::coins_extended::{ETH, DAI, USDC, register_coins, mint_coin, burn_coin};
    ...
    fun test_burn_coin(token_admin: signer, test_account: signer) {
        ...
        let eth_coins = coin::withdraw<ETH>(&test_account, 100000000);
        burn_coin(&token_admin, eth_coins);

        let usdc_coins = coin::withdraw<USDC>(&test_account, 1000000);
        burn_coin(&token_admin, usdc_coins);

        let dai_coins = coin::withdraw<DAI>(&test_account, 1000000);
        burn_coin(&token_admin, dai_coins);
    }
}